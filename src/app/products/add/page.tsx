...
import { PackagePlus, ArrowLeft, Tag, Barcode, Palette, Boxes, IndianRupee, Shirt, AlertCircle } from "lucide-react";
...
export default function AddProductPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const res = await fetch("/api/products", {
      ...
      body: JSON.stringify({ ...form, stock: parseInt(form.stock as string) || 0, price: parseFloat(form.price as string) || 0 }),
    });
  ...
  const sizes = [
    { value: "S", label: "Small", abbr: "S", color: "from-cyan-500 to-blue-500", shadow: "shadow-cyan-500/25" },
    { value: "M", label: "Medium", abbr: "M", color: "from-violet-500 to-purple-500", shadow: "shadow-violet-500/25" },
    { value: "L", label: "Large", abbr: "L", color: "from-pink-500 to-rose-500", shadow: "shadow-pink-500/25" },
  ];
        ...
        <main className="flex-1 lg:ml-0 p-4 sm:p-6 lg:p-8 pt-16 lg:pt-8 overflow-hidden relative">
          ...
          <div className="max-w-2xl mx-auto relative z-10">
            ...
            >
              ...
              {/* Size Selector */}
              <div>
                <label className="flex items-center gap-2 text-xs font-semibold text-muted mb-3 uppercase tracking-wider">
                ...
                <div className="grid grid-cols-3 gap-3">
                  {sizes.map((s) => (
                    <button
                      ...
                      className={`group relative py-4 rounded-xl text-sm font-semibold transition-all duration-300 ${
                        form.size === s.value
                          ? `bg-gradient-to-br ${s.color} text-white shadow-lg ${s.shadow} scale-[1.03]`
                          : "bg-background/80 text-muted border border-border hover:border-indigo-300 dark:hover:border-indigo-700 hover:bg-card-hover hover:scale-[1.01]"
                      }`}
                    >
                      <span className={`text-xs block mb-0.5 ${form.size === s.value ? "text-white/70" : "text-muted/50"}`}>
                        {s.value}
                      </span>
                      <span>{s.label}</span>
                    </button>
